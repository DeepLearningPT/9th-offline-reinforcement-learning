...
with addition of support for saving videos of
...
def interact_with_environment(env, state_dim, action_dim, max_action, device, args):
	...
	if args.generate_buffer or args.generate_video:
		print(f"Loading Policy {setting}")
	...
	for t in range(int(args.max_timesteps)):
		...
		done_bool = float(done) if episode_timesteps < env_wrapped._max_episode_steps else 0
		...
		# Evaluate episode
		if args.train_behavioral and (t + 1) % args.eval_freq == 0 and not args.generate_video:
...
if __name__ == "__main__":
	...
	parser.add_argument("--generate_video", action="store_true")    # Just store a video
	...
	if args.train_behavioral or args.generate_buffer or args.generate_video:
		interact_with_environment(env, state_dim, action_dim, max_action, device, args)
	else:
		bcq_model = train_BCQ(state_dim, action_dim, max_action, device, args)
		bcq_model.save(f"./models/bcq_{args.env}_{args.seed}")
